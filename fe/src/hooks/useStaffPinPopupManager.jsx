...
const StaffPinPopupManager = ({ showTestButton, closeUserPinPopup }) => {
  ...
  // Track triple touch to open only (not toggle)
  useTouchDetection({
    onTripleTouch: () => {
      if (!visible) {
    ...
    <>
      ...
      {showTestButton && (
        <button
          ...
          onClick={() => {
            setVisible(true);
            if (typeof closeUserPinPopup === "function")
              closeUserPinPopup(true); // true = close user popup
          }}
        >
          TEST STAFF PIN
        </button>
      ...
      {visible && (
        <PinPopup
          ...
          onClose={() => {
            setVisible(false);
            if (typeof closeUserPinPopup === "function")
              closeUserPinPopup(false); // เปิด user popup กลับ
          }}
        />
      )}
    </>
